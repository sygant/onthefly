...
#include "Common.h"
#include "../Workflow/Reconstructor.h"
...
ModelViewerWidget::ModelViewerWidget(QWidget* parent) : QOpenGLWidget(parent), mouse_is_pressed_(false), focus_distance_(kInitFocusDistance), selected_image_id_(kInvalidImageId), selected_point3D_id_(kInvalidPoint3DId), coordinate_grid_enabled_(true), near_plane_(kInitNearPlane) 
{
...
void ModelViewerWidget::ReloadReconstruction() {
    ...
    __try
    {
        ...
        QString labelString = QString::fromStdString(formatDuration(reconstruction->reconstructTime / 1000));
        labelString += QString().asprintf("  %d Images - %d Points", static_cast<int>(reg_image_ids.size()), static_cast<int>(points3D.size()));

        statusbar_status_label->setText(labelString);
...
void ModelViewerWidget::UploadImageData(const bool selection_mode) {
    ...
    for (const image_t image_id : reg_image_ids)
    {
        ...
        else
        {
            ...
            else
            {
                if (database)
                {
                    ...
                    plane_color = database->CalculatePlaneColor(image_id);
                }
        ...
        BuildImageModel(image, camera, image_size_, plane_color, frame_color, &triangle_data, selection_mode ? nullptr : &line_data);
    }
