...
from doppelgangers.utils.process_database import create_image_pair_list
...
def main_worker(gpu, ngpus_per_node, cfg, args): 
    ...
    result = np.load(os.path.join(cfg.data.output_path, "pair_probability_list.npy"), allow_pickle=True).item()
    ...
    for i in range(len(pairs_id)):
        if y_scores[i] < args.threshold:
            match_pair_list.append(pairs_id[i])
    # print(y_scores)
    print(f"{len(match_pair_list)}/{len(pairs_id)} failed")
