...
std::string StringArgParse(const std::string& initPath, const std::string& key)
{
    std::fstream file(initPath);
    std::string line;
    while (std::getline(file,line))
    {
        ...
        if (pos != std::string::npos)
        {
            ...
            if (pos != std::string::npos)
            {
                ...
                auto valuepos = line.find_last_not_of("\t");
                return line.substr(pos, valuepos - pos);
            }
        }
    }
    return std::string();
