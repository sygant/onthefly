...
void IncrementalMapper::BeginReconstruction(
    const std::shared_ptr<Reconstruction>& reconstruction, std::unordered_set<image_t>* regFailedImages) {
    ...
    if (regFailedImages)
    {
        this->regFailedImages = regFailedImages;
    }
}
...
bool IncrementalMapper::FindInitialImagePair(const Options& options,
    ...
    for (size_t i1 = 0; i1 < image_ids1.size(); ++i1) {
        if (regFailedImages && regFailedImages->find(image_ids1[i1]) == regFailedImages->end())
        {
            continue;
        }
        *image_id1 = image_ids1[i1];
        ...
        for (size_t i2 = 0; i2 < image_ids2.size(); ++i2) {
            if (regFailedImages && regFailedImages->find(image_ids2[i2]) == regFailedImages->end())
            {
                continue;
            }
...
void IncrementalMapper::RegisterImageEvent(const image_t image_id) {
    ...
    if (regFailedImages && regFailedImages->find(image_id) != regFailedImages->end())
    {
        regFailedImages->erase(image_id);
    }
}
...
void IncrementalMapper::DeRegisterImageEvent(const image_t image_id) {
    ...
    if (regFailedImages)
    {
        regFailedImages->insert(image_id);
    }
}
